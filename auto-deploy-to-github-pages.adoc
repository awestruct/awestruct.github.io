...
Next, rerun the encryption command with --add at the end. This will
append the following configuration to the +.travis.yml+ file,
replacing +<encrypted_string>+ with the value in quotes outputted by
the +travis encrypt+ command:
