...
Most of the code involves configuring git so that Awestruct can commit and push the changes to the publish branch, using the HTTPS protocol along with a token-based authentification.
...
That defines the secure environment variables, which Travis will unecrypt at the start of the build, so that they can be used by the build task (in our case, +rake travis+).
